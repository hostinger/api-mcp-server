...
export interface APITools {
  ...
  "agency-hosting_deployPhpApplication": {
    params: {
      ...
      domain: string;
      /**
       * Absolute or relative path to the website archive file. Supported formats: zip, tar, tar.gz, tgz. If user provides a directory path, create an archive from it before proceeding using EXACTLY this naming pattern: directoryname_YYYYMMDD_HHMMSS.zip (e.g., mywebsite_20250115_143022.zip)
       */
      archivePath: string;
      /**
       * Whether to remove the local archive file after successful deployment (default: true)
...
and `user` are identifiers, not secrets.
  ...
  "hosting_setupWebsiteDatabaseV1": {
    params: {
      ...
      domain: string;
      /**
       * Optional database name. Generated when omitted. Letters, digits and underscores;
...
the full name is 12 to 25 characters.
      ...
      name?: string;
      /**
       * Optional database user. Generated when omitted. Letters, digits and underscores;
must not start with an underscore. Up to 14 characters without the account username
prefix (`u123456789_`), which is added automatically when missing. With the prefix
